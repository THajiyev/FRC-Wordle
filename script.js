const frc_teams = {
    "4": {
        "rookie_year": 1997,
        "location": "California",
        "epa_rank": 1025,
        "division": "Archimedes"
    },
    "11": {
        "rookie_year": 1997,
        "location": "New Jersey",
        "epa_rank": 171,
        "division": "Newton"
    },
    "16": {
        "rookie_year": 1996,
        "location": "Arkansas",
        "epa_rank": 200,
        "division": "Johnson"
    },
    "20": {
        "rookie_year": 1992,
        "location": "New York",
        "epa_rank": 201,
        "division": "Daly"
    },
    "25": {
        "rookie_year": 1997,
        "location": "New Jersey",
        "epa_rank": 314,
        "division": "Milstein"
    },
    "27": {
        "rookie_year": 1997,
        "location": "Michigan",
        "epa_rank": 361,
        "division": "Archimedes"
    },
    "28": {
        "rookie_year": 1996,
        "location": "New York",
        "epa_rank": 2621,
        "division": "Johnson"
    },
    "33": {
        "rookie_year": 1996,
        "location": "Michigan",
        "epa_rank": 130,
        "division": "Archimedes"
    },
    "51": {
        "rookie_year": 1996,
        "location": "Michigan",
        "epa_rank": 41,
        "division": "Johnson"
    },
    "59": {
        "rookie_year": 1997,
        "location": "Florida",
        "epa_rank": 129,
        "division": "Galileo"
    },
    "67": {
        "rookie_year": 1997,
        "location": "Michigan",
        "epa_rank": 58,
        "division": "Galileo"
    },
    "68": {
        "rookie_year": 1998,
        "location": "Michigan",
        "epa_rank": 1205,
        "division": "Hopper"
    },
    "70": {
        "rookie_year": 1998,
        "location": "Michigan",
        "epa_rank": 99,
        "division": "Archimedes"
    },
    "78": {
        "rookie_year": 1996,
        "location": "Rhode Island",
        "epa_rank": 374,
        "division": "Archimedes"
    },
    "85": {
        "rookie_year": 1996,
        "location": "Michigan",
        "epa_rank": 246,
        "division": "Johnson"
    },
    "88": {
        "rookie_year": 1996,
        "location": "Massachusetts",
        "epa_rank": 168,
        "division": "Johnson"
    },
    "100": {
        "rookie_year": 1995,
        "location": "California",
        "epa_rank": 760,
        "division": "Daly"
    },
    "103": {
        "rookie_year": 1997,
        "location": "Pennsylvania",
        "epa_rank": 96,
        "division": "Galileo"
    },
    "107": {
        "rookie_year": 1997,
        "location": "Michigan",
        "epa_rank": 407,
        "division": "Hopper"
    },
    "111": {
        "rookie_year": 1996,
        "location": "Illinois",
        "epa_rank": 6,
        "division": "Archimedes"
    },
    "115": {
        "rookie_year": 1998,
        "location": "California",
        "epa_rank": 178,
        "division": "Curie"
    },
    "118": {
        "rookie_year": 1997,
        "location": "Texas",
        "epa_rank": 38,
        "division": "Archimedes"
    },
    "120": {
        "rookie_year": 1995,
        "location": "Ohio",
        "epa_rank": 566,
        "division": "Archimedes"
    },
    "125": {
        "rookie_year": 1998,
        "location": "Massachusetts",
        "epa_rank": 83,
        "division": "Daly"
    },
    "135": {
        "rookie_year": 1998,
        "location": "Indiana",
        "epa_rank": 803,
        "division": "Milstein"
    },
    "144": {
        "rookie_year": 1994,
        "location": "Ohio",
        "epa_rank": 686,
        "division": "Daly"
    },
    "148": {
        "rookie_year": 1992,
        "location": "Texas",
        "epa_rank": 62,
        "division": "Curie"
    },
    "157": {
        "rookie_year": 1992,
        "location": "Massachusetts",
        "epa_rank": 1562,
        "division": "Galileo"
    },
    "166": {
        "rookie_year": 1995,
        "location": "New Hampshire",
        "epa_rank": 462,
        "division": "Johnson"
    },
    "176": {
        "rookie_year": 1996,
        "location": "Connecticut",
        "epa_rank": 28,
        "division": "Milstein"
    },
    "177": {
        "rookie_year": 1995,
        "location": "Connecticut",
        "epa_rank": 95,
        "division": "Newton"
    },
    "179": {
        "rookie_year": 1998,
        "location": "Florida",
        "epa_rank": 160,
        "division": "Newton"
    },
    "180": {
        "rookie_year": 1998,
        "location": "Florida",
        "epa_rank": 135,
        "division": "Curie"
    },
    "190": {
        "rookie_year": 1992,
        "location": "Massachusetts",
        "epa_rank": 1465,
        "division": "Hopper"
    },
    "195": {
        "rookie_year": 1998,
        "location": "Connecticut",
        "epa_rank": 33,
        "division": "Newton"
    },
    "201": {
        "rookie_year": 1998,
        "location": "Michigan",
        "epa_rank": 1326,
        "division": "Hopper"
    },
    "203": {
        "rookie_year": 1998,
        "location": "New Jersey",
        "epa_rank": 577,
        "division": "Daly"
    },
    "217": {
        "rookie_year": 1999,
        "location": "Michigan",
        "epa_rank": 820,
        "division": "Johnson"
    },
    "226": {
        "rookie_year": 1999,
        "location": "Michigan",
        "epa_rank": 485,
        "division": "Hopper"
    },
    "230": {
        "rookie_year": 1999,
        "location": "Connecticut",
        "epa_rank": 114,
        "division": "Daly"
    },
    "245": {
        "rookie_year": 1999,
        "location": "Michigan",
        "epa_rank": 154,
        "division": "Hopper"
    },
    "254": {
        "rookie_year": 1999,
        "location": "California",
        "epa_rank": 3,
        "division": "Archimedes"
    },
    "281": {
        "rookie_year": 1999,
        "location": "South Carolina",
        "epa_rank": 354,
        "division": "Johnson"
    },
    "302": {
        "rookie_year": 1999,
        "location": "Michigan",
        "epa_rank": 118,
        "division": "Curie"
    },
    "316": {
        "rookie_year": 1999,
        "location": "New Jersey",
        "epa_rank": 84,
        "division": "Milstein"
    },
    "319": {
        "rookie_year": 1999,
        "location": "New Hampshire",
        "epa_rank": 96,
        "division": "Milstein"
    },
    "321": {
        "rookie_year": 1999,
        "location": "Pennsylvania",
        "epa_rank": 489,
        "division": "Daly"
    },
    "324": {
        "rookie_year": 1998,
        "location": "Texas",
        "epa_rank": 274,
        "division": "Archimedes"
    },
    "325": {
        "rookie_year": 1998,
        "location": "Ohio",
        "epa_rank": 163,
        "division": "Curie"
    },
    "333": {
        "rookie_year": 1999,
        "location": "New York",
        "epa_rank": 433,
        "division": "Curie"
    },
    "341": {
        "rookie_year": 2000,
        "location": "Pennsylvania",
        "epa_rank": 187,
        "division": "Hopper"
    },
    "343": {
        "rookie_year": 2000,
        "location": "South Carolina",
        "epa_rank": 283,
        "division": "Curie"
    },
    "353": {
        "rookie_year": 2000,
        "location": "New York",
        "epa_rank": 595,
        "division": "Hopper"
    },
    "359": {
        "rookie_year": 2000,
        "location": "Hawaii",
        "epa_rank": 116,
        "division": "Johnson"
    },
    "360": {
        "rookie_year": 2000,
        "location": "Washington",
        "epa_rank": 332,
        "division": "Milstein"
    },
    "365": {
        "rookie_year": 2000,
        "location": "Delaware",
        "epa_rank": 662,
        "division": "Hopper"
    },
    "384": {
        "rookie_year": 2000,
        "location": "Virginia",
        "epa_rank": 326,
        "division": "Milstein"
    },
    "399": {
        "rookie_year": 2000,
        "location": "California",
        "epa_rank": 993,
        "division": "Curie"
    },
    "401": {
        "rookie_year": 2000,
        "location": "Virginia",
        "epa_rank": 298,
        "division": "Galileo"
    },
    "422": {
        "rookie_year": 2000,
        "location": "Virginia",
        "epa_rank": 185,
        "division": "Johnson"
    },
    "449": {
        "rookie_year": 2000,
        "location": "Maryland",
        "epa_rank": 298,
        "division": "Curie"
    },
    "461": {
        "rookie_year": 2000,
        "location": "Indiana",
        "epa_rank": 80,
        "division": "Galileo"
    },
    "469": {
        "rookie_year": 2000,
        "location": "Michigan",
        "epa_rank": 343,
        "division": "Milstein"
    },
    "484": {
        "rookie_year": 2000,
        "location": "Pennsylvania",
        "epa_rank": 291,
        "division": "Johnson"
    },
    "488": {
        "rookie_year": 2000,
        "location": "Washington",
        "epa_rank": 138,
        "division": "Hopper"
    },
    "494": {
        "rookie_year": 2001,
        "location": "Michigan",
        "epa_rank": 109,
        "division": "Newton"
    },
    "498": {
        "rookie_year": 2001,
        "location": "Arizona",
        "epa_rank": 444,
        "division": "Hopper"
    },
    "503": {
        "rookie_year": 2001,
        "location": "Michigan",
        "epa_rank": 410,
        "division": "Newton"
    },
    "533": {
        "rookie_year": 2001,
        "location": "New York",
        "epa_rank": 1211,
        "division": "Johnson"
    },
    "548": {
        "rookie_year": 2001,
        "location": "Michigan",
        "epa_rank": 509,
        "division": "Archimedes"
    },
    "573": {
        "rookie_year": 2001,
        "location": "Michigan",
        "epa_rank": 325,
        "division": "Hopper"
    },
    "587": {
        "rookie_year": 2001,
        "location": "North Carolina",
        "epa_rank": 241,
        "division": "Milstein"
    },
    "597": {
        "rookie_year": 2001,
        "location": "California",
        "epa_rank": 460,
        "division": "Milstein"
    },
    "599": {
        "rookie_year": 2001,
        "location": "California",
        "epa_rank": 313,
        "division": "Galileo"
    },
    "604": {
        "rookie_year": 2001,
        "location": "California",
        "epa_rank": 37,
        "division": "Milstein"
    },
    "610": {
        "rookie_year": 2001,
        "location": "Canada",
        "epa_rank": 301,
        "division": "Hopper"
    },
    "624": {
        "rookie_year": 2001,
        "location": "Texas",
        "epa_rank": 86,
        "division": "Archimedes"
    },
    "649": {
        "rookie_year": 2001,
        "location": "California",
        "epa_rank": 338,
        "division": "Curie"
    },
    "668": {
        "rookie_year": 2001,
        "location": "California",
        "epa_rank": 1193,
        "division": "Johnson"
    },
    "687": {
        "rookie_year": 2001,
        "location": "California",
        "epa_rank": 199,
        "division": "Archimedes"
    },
    "694": {
        "rookie_year": 2001,
        "location": "New York",
        "epa_rank": 52,
        "division": "Milstein"
    },
    "695": {
        "rookie_year": 2001,
        "location": "Ohio",
        "epa_rank": 66,
        "division": "Daly"
    },
    "696": {
        "rookie_year": 2001,
        "location": "California",
        "epa_rank": 150,
        "division": "Hopper"
    },
    "702": {
        "rookie_year": 2001,
        "location": "California",
        "epa_rank": 1886,
        "division": "Galileo"
    },
    "772": {
        "rookie_year": 2002,
        "location": "Canada",
        "epa_rank": 1455,
        "division": "Hopper"
    },
    "818": {
        "rookie_year": 2002,
        "location": "Michigan",
        "epa_rank": 272,
        "division": "Curie"
    },
    "834": {
        "rookie_year": 2002,
        "location": "Pennsylvania",
        "epa_rank": 300,
        "division": "Archimedes"
    },
    "836": {
        "rookie_year": 2002,
        "location": "Maryland",
        "epa_rank": 407,
        "division": "Galileo"
    },
    "840": {
        "rookie_year": 2002,
        "location": "California",
        "epa_rank": 1402,
        "division": "Curie"
    },
    "857": {
        "rookie_year": 2002,
        "location": "Michigan",
        "epa_rank": 176,
        "division": "Newton"
    },
    "862": {
        "rookie_year": 2002,
        "location": "Michigan",
        "epa_rank": 284,
        "division": "Archimedes"
    },
    "870": {
        "rookie_year": 2002,
        "location": "New York",
        "epa_rank": 92,
        "division": "Daly"
    },
    "876": {
        "rookie_year": 2002,
        "location": "North Dakota",
        "epa_rank": 144,
        "division": "Hopper"
    },
    "900": {
        "rookie_year": 2002,
        "location": "North Carolina",
        "epa_rank": 232,
        "division": "Newton"
    },
    "910": {
        "rookie_year": 2002,
        "location": "Michigan",
        "epa_rank": 712,
        "division": "Milstein"
    },
    "930": {
        "rookie_year": 2002,
        "location": "Wisconsin",
        "epa_rank": 10,
        "division": "Milstein"
    },
    "948": {
        "rookie_year": 2002,
        "location": "Washington",
        "epa_rank": 384,
        "division": "Galileo"
    },
    "955": {
        "rookie_year": 2002,
        "location": "Oregon",
        "epa_rank": 435,
        "division": "Newton"
    },
    "968": {
        "rookie_year": 2002,
        "location": "California",
        "epa_rank": 642,
        "division": "Galileo"
    },
    "971": {
        "rookie_year": 2002,
        "location": "California",
        "epa_rank": 71,
        "division": "Johnson"
    },
    "972": {
        "rookie_year": 2002,
        "location": "California",
        "epa_rank": 77,
        "division": "Archimedes"
    },
    "973": {
        "rookie_year": 2002,
        "location": "California",
        "epa_rank": 50,
        "division": "Johnson"
    },
    "987": {
        "rookie_year": 2002,
        "location": "Nevada",
        "epa_rank": 67,
        "division": "Milstein"
    },
    "999": {
        "rookie_year": 2002,
        "location": "Connecticut",
        "epa_rank": 695,
        "division": "Hopper"
    },
    "1023": {
        "rookie_year": 2003,
        "location": "Michigan",
        "epa_rank": 351,
        "division": "Newton"
    },
    "1025": {
        "rookie_year": 2003,
        "location": "Michigan",
        "epa_rank": 622,
        "division": "Johnson"
    },
    "1038": {
        "rookie_year": 2003,
        "location": "Ohio",
        "epa_rank": 701,
        "division": "Daly"
    },
    "1072": {
        "rookie_year": 2003,
        "location": "California",
        "epa_rank": 596,
        "division": "Milstein"
    },
    "1100": {
        "rookie_year": 2003,
        "location": "Massachusetts",
        "epa_rank": 191,
        "division": "Curie"
    },
    "1102": {
        "rookie_year": 2003,
        "location": "South Carolina",
        "epa_rank": 122,
        "division": "Archimedes"
    },
    "1114": {
        "rookie_year": 2003,
        "location": "Canada",
        "epa_rank": 53,
        "division": "Galileo"
    },
    "1123": {
        "rookie_year": 2003,
        "location": "Virginia",
        "epa_rank": 401,
        "division": "Newton"
    },
    "1138": {
        "rookie_year": 2003,
        "location": "California",
        "epa_rank": 754,
        "division": "Daly"
    },
    "1156": {
        "rookie_year": 2003,
        "location": "Brazil",
        "epa_rank": 249,
        "division": "Newton"
    },
    "1165": {
        "rookie_year": 2003,
        "location": "Arizona",
        "epa_rank": 1667,
        "division": "Galileo"
    },
    "1189": {
        "rookie_year": 2003,
        "location": "Michigan",
        "epa_rank": 194,
        "division": "Archimedes"
    },
    "1218": {
        "rookie_year": 2003,
        "location": "Pennsylvania",
        "epa_rank": 484,
        "division": "Milstein"
    },
    "1241": {
        "rookie_year": 2004,
        "location": "Canada",
        "epa_rank": 159,
        "division": "Milstein"
    },
    "1296": {
        "rookie_year": 2004,
        "location": "Texas",
        "epa_rank": 367,
        "division": "Johnson"
    },
    "1311": {
        "rookie_year": 2004,
        "location": "Georgia",
        "epa_rank": 2537,
        "division": "Hopper"
    },
    "1318": {
        "rookie_year": 2004,
        "location": "Washington",
        "epa_rank": 131,
        "division": "Galileo"
    },
    "1323": {
        "rookie_year": 2004,
        "location": "California",
        "epa_rank": 1,
        "division": "Hopper"
    },
    "1325": {
        "rookie_year": 2004,
        "location": "Canada",
        "epa_rank": 23,
        "division": "Daly"
    },
    "1339": {
        "rookie_year": 2004,
        "location": "Colorado",
        "epa_rank": 214,
        "division": "Johnson"
    },
    "1391": {
        "rookie_year": 2004,
        "location": "Pennsylvania",
        "epa_rank": 275,
        "division": "Johnson"
    },
    "1403": {
        "rookie_year": 2004,
        "location": "New Jersey",
        "epa_rank": 182,
        "division": "Curie"
    },
    "1414": {
        "rookie_year": 2004,
        "location": "Georgia",
        "epa_rank": 340,
        "division": "Milstein"
    },
    "1425": {
        "rookie_year": 2004,
        "location": "Oregon",
        "epa_rank": 203,
        "division": "Johnson"
    },
    "1466": {
        "rookie_year": 2004,
        "location": "Tennessee",
        "epa_rank": 330,
        "division": "Newton"
    },
    "1468": {
        "rookie_year": 2004,
        "location": "New York",
        "epa_rank": 210,
        "division": "Newton"
    },
    "1477": {
        "rookie_year": 2004,
        "location": "Texas",
        "epa_rank": 320,
        "division": "Daly"
    },
    "1481": {
        "rookie_year": 2004,
        "location": "Michigan",
        "epa_rank": 622,
        "division": "Archimedes"
    },
    "1501": {
        "rookie_year": 2005,
        "location": "Indiana",
        "epa_rank": 302,
        "division": "Newton"
    },
    "1506": {
        "rookie_year": 2005,
        "location": "Michigan",
        "epa_rank": 69,
        "division": "Milstein"
    },
    "1511": {
        "rookie_year": 2005,
        "location": "New York",
        "epa_rank": 932,
        "division": "Daly"
    },
    "1533": {
        "rookie_year": 2005,
        "location": "North Carolina",
        "epa_rank": 451,
        "division": "Daly"
    },
    "1538": {
        "rookie_year": 2005,
        "location": "California",
        "epa_rank": 16,
        "division": "Newton"
    },
    "1540": {
        "rookie_year": 2005,
        "location": "Oregon",
        "epa_rank": 290,
        "division": "Johnson"
    },
    "1561": {
        "rookie_year": 2005,
        "location": "Oklahoma",
        "epa_rank": 366,
        "division": "Archimedes"
    },
    "1574": {
        "rookie_year": 2005,
        "location": "Israel",
        "epa_rank": 322,
        "division": "Archimedes"
    },
    "1577": {
        "rookie_year": 2005,
        "location": "Israel",
        "epa_rank": 19,
        "division": "Archimedes"
    },
    "1591": {
        "rookie_year": 2005,
        "location": "New York",
        "epa_rank": 40,
        "division": "Galileo"
    },
    "1599": {
        "rookie_year": 2005,
        "location": "Virginia",
        "epa_rank": 348,
        "division": "Milstein"
    },
    "1619": {
        "rookie_year": 2005,
        "location": "Colorado",
        "epa_rank": 29,
        "division": "Archimedes"
    },
    "1622": {
        "rookie_year": 2005,
        "location": "California",
        "epa_rank": 872,
        "division": "Galileo"
    },
    "1625": {
        "rookie_year": 2005,
        "location": "Illinois",
        "epa_rank": 390,
        "division": "Johnson"
    },
    "1629": {
        "rookie_year": 2005,
        "location": "Maryland",
        "epa_rank": 527,
        "division": "Newton"
    },
    "1676": {
        "rookie_year": 2005,
        "location": "New Jersey",
        "epa_rank": 196,
        "division": "Johnson"
    },
    "1678": {
        "rookie_year": 2005,
        "location": "California",
        "epa_rank": 9,
        "division": "Galileo"
    },
    "1683": {
        "rookie_year": 2005,
        "location": "Georgia",
        "epa_rank": 216,
        "division": "Milstein"
    },
    "1684": {
        "rookie_year": 2005,
        "location": "Michigan",
        "epa_rank": 261,
        "division": "Johnson"
    },
    "1690": {
        "rookie_year": 2005,
        "location": "Israel",
        "epa_rank": 85,
        "division": "Galileo"
    },
    "1700": {
        "rookie_year": 2005,
        "location": "California",
        "epa_rank": 998,
        "division": "Archimedes"
    },
    "1701": {
        "rookie_year": 2005,
        "location": "Michigan",
        "epa_rank": 280,
        "division": "Galileo"
    },
    "1706": {
        "rookie_year": 2005,
        "location": "Missouri",
        "epa_rank": 89,
        "division": "Hopper"
    },
    "1708": {
        "rookie_year": 2005,
        "location": "Pennsylvania",
        "epa_rank": 1102,
        "division": "Galileo"
    },
    "1710": {
        "rookie_year": 2006,
        "location": "Kansas",
        "epa_rank": 863,
        "division": "Daly"
    },
    "1714": {
        "rookie_year": 2006,
        "location": "Wisconsin",
        "epa_rank": 421,
        "division": "Johnson"
    },
    "1718": {
        "rookie_year": 2006,
        "location": "Michigan",
        "epa_rank": 63,
        "division": "Johnson"
    },
    "1727": {
        "rookie_year": 2006,
        "location": "Maryland",
        "epa_rank": 104,
        "division": "Hopper"
    },
    "1730": {
        "rookie_year": 2006,
        "location": "Missouri",
        "epa_rank": 128,
        "division": "Johnson"
    },
    "1731": {
        "rookie_year": 2006,
        "location": "Virginia",
        "epa_rank": 54,
        "division": "Daly"
    },
    "1732": {
        "rookie_year": 2006,
        "location": "Wisconsin",
        "epa_rank": 158,
        "division": "Johnson"
    },
    "1745": {
        "rookie_year": 2006,
        "location": "Texas",
        "epa_rank": 170,
        "division": "Milstein"
    },
    "1746": {
        "rookie_year": 2006,
        "location": "Georgia",
        "epa_rank": 203,
        "division": "Newton"
    },
    "1756": {
        "rookie_year": 2006,
        "location": "Illinois",
        "epa_rank": 36,
        "division": "Curie"
    },
    "1757": {
        "rookie_year": 2006,
        "location": "Massachusetts",
        "epa_rank": 139,
        "division": "Newton"
    },
    "1768": {
        "rookie_year": 2006,
        "location": "Massachusetts",
        "epa_rank": 112,
        "division": "Archimedes"
    },
    "1771": {
        "rookie_year": 2006,
        "location": "Georgia",
        "epa_rank": 120,
        "division": "Curie"
    },
    "1781": {
        "rookie_year": 2006,
        "location": "Illinois",
        "epa_rank": 179,
        "division": "Daly"
    },
    "1785": {
        "rookie_year": 2006,
        "location": "Missouri",
        "epa_rank": 389,
        "division": "Johnson"
    },
    "1792": {
        "rookie_year": 2006,
        "location": "Wisconsin",
        "epa_rank": 829,
        "division": "Hopper"
    },
    "1796": {
        "rookie_year": 2006,
        "location": "New York",
        "epa_rank": 219,
        "division": "Hopper"
    },
    "1799": {
        "rookie_year": 2006,
        "location": "Colorado",
        "epa_rank": 1088,
        "division": "Hopper"
    },
    "1802": {
        "rookie_year": 2006,
        "location": "Kansas",
        "epa_rank": 490,
        "division": "Johnson"
    },
    "1807": {
        "rookie_year": 2006,
        "location": "New Jersey",
        "epa_rank": 363,
        "division": "Hopper"
    },
    "1816": {
        "rookie_year": 2006,
        "location": "Minnesota",
        "epa_rank": 554,
        "division": "Newton"
    },
    "1836": {
        "rookie_year": 2006,
        "location": "California",
        "epa_rank": 710,
        "division": "Newton"
    },
    "1868": {
        "rookie_year": 2006,
        "location": "California",
        "epa_rank": 114,
        "division": "Curie"
    },
    "1884": {
        "rookie_year": 2006,
        "location": "United Kingdom",
        "epa_rank": 1223,
        "division": "Archimedes"
    },
    "1902": {
        "rookie_year": 2006,
        "location": "Florida",
        "epa_rank": 962,
        "division": "Galileo"
    },
    "1922": {
        "rookie_year": 2006,
        "location": "New Hampshire",
        "epa_rank": 523,
        "division": "Daly"
    },
    "1923": {
        "rookie_year": 2006,
        "location": "New Jersey",
        "epa_rank": 48,
        "division": "Milstein"
    },
    "1983": {
        "rookie_year": 2007,
        "location": "Washington",
        "epa_rank": 432,
        "division": "Milstein"
    },
    "1986": {
        "rookie_year": 2007,
        "location": "Missouri",
        "epa_rank": 173,
        "division": "Hopper"
    },
    "1987": {
        "rookie_year": 2007,
        "location": "Missouri",
        "epa_rank": 27,
        "division": "Hopper"
    },
    "2046": {
        "rookie_year": 2007,
        "location": "Washington",
        "epa_rank": 17,
        "division": "Hopper"
    },
    "2052": {
        "rookie_year": 2007,
        "location": "Minnesota",
        "epa_rank": 125,
        "division": "Hopper"
    },
    "2054": {
        "rookie_year": 2007,
        "location": "Michigan",
        "epa_rank": 282,
        "division": "Daly"
    },
    "2056": {
        "rookie_year": 2007,
        "location": "Canada",
        "epa_rank": 2,
        "division": "Archimedes"
    },
    "2059": {
        "rookie_year": 2007,
        "location": "North Carolina",
        "epa_rank": 1064,
        "division": "Johnson"
    },
    "2067": {
        "rookie_year": 2007,
        "location": "Connecticut",
        "epa_rank": 257,
        "division": "Johnson"
    },
    "2073": {
        "rookie_year": 2007,
        "location": "California",
        "epa_rank": 219,
        "division": "Daly"
    },
    "2075": {
        "rookie_year": 2007,
        "location": "Michigan",
        "epa_rank": 25,
        "division": "Johnson"
    },
    "2080": {
        "rookie_year": 2007,
        "location": "Louisiana",
        "epa_rank": 1325,
        "division": "Johnson"
    },
    "2096": {
        "rookie_year": 2007,
        "location": "Israel",
        "epa_rank": 582,
        "division": "Hopper"
    },
    "2106": {
        "rookie_year": 2007,
        "location": "Virginia",
        "epa_rank": 809,
        "division": "Archimedes"
    },
    "2169": {
        "rookie_year": 2007,
        "location": "Minnesota",
        "epa_rank": 356,
        "division": "Hopper"
    },
    "2170": {
        "rookie_year": 2007,
        "location": "Connecticut",
        "epa_rank": 750,
        "division": "Johnson"
    },
    "2194": {
        "rookie_year": 2007,
        "location": "Wisconsin",
        "epa_rank": 399,
        "division": "Galileo"
    },
    "2199": {
        "rookie_year": 2007,
        "location": "Maryland",
        "epa_rank": 372,
        "division": "Curie"
    },
    "2202": {
        "rookie_year": 2007,
        "location": "Wisconsin",
        "epa_rank": 519,
        "division": "Archimedes"
    },
    "2220": {
        "rookie_year": 2007,
        "location": "Minnesota",
        "epa_rank": 795,
        "division": "Archimedes"
    },
    "2230": {
        "rookie_year": 2007,
        "location": "Israel",
        "epa_rank": 471,
        "division": "Milstein"
    },
    "2240": {
        "rookie_year": 2007,
        "location": "Colorado",
        "epa_rank": 260,
        "division": "Milstein"
    },
    "2264": {
        "rookie_year": 2007,
        "location": "Minnesota",
        "epa_rank": 319,
        "division": "Daly"
    },
    "2333": {
        "rookie_year": 2008,
        "location": "Oklahoma",
        "epa_rank": 976,
        "division": "Daly"
    },
    "2337": {
        "rookie_year": 2008,
        "location": "Michigan",
        "epa_rank": 72,
        "division": "Galileo"
    },
    "2338": {
        "rookie_year": 2008,
        "location": "Illinois",
        "epa_rank": 32,
        "division": "Hopper"
    },
    "2341": {
        "rookie_year": 2008,
        "location": "Oklahoma",
        "epa_rank": 1736,
        "division": "Hopper"
    },
    "2359": {
        "rookie_year": 2008,
        "location": "Oklahoma",
        "epa_rank": 1639,
        "division": "Johnson"
    },
    "2363": {
        "rookie_year": 2008,
        "location": "Virginia",
        "epa_rank": 34,
        "division": "Daly"
    },
    "2370": {
        "rookie_year": 2008,
        "location": "Vermont",
        "epa_rank": 480,
        "division": "Curie"
    },
    "2383": {
        "rookie_year": 2008,
        "location": "Florida",
        "epa_rank": 783,
        "division": "Galileo"
    },
    "2399": {
        "rookie_year": 2008,
        "location": "Ohio",
        "epa_rank": 218,
        "division": "Milstein"
    },
    "2403": {
        "rookie_year": 2008,
        "location": "Arizona",
        "epa_rank": 492,
        "division": "Milstein"
    },
    "2412": {
        "rookie_year": 2008,
        "location": "Washington",
        "epa_rank": 370,
        "division": "Daly"
    },
    "2429": {
        "rookie_year": 2008,
        "location": "California",
        "epa_rank": 706,
        "division": "Johnson"
    },
    "2438": {
        "rookie_year": 2008,
        "location": "Hawaii",
        "epa_rank": 557,
        "division": "Daly"
    },
    "2445": {
        "rookie_year": 2008,
        "location": "Hawaii",
        "epa_rank": 1056,
        "division": "Daly"
    },
    "2451": {
        "rookie_year": 2008,
        "location": "Illinois",
        "epa_rank": 281,
        "division": "Curie"
    },
    "2457": {
        "rookie_year": 2008,
        "location": "Missouri",
        "epa_rank": 1155,
        "division": "Archimedes"
    },
    "2468": {
        "rookie_year": 2008,
        "location": "Texas",
        "epa_rank": 12,
        "division": "Archimedes"
    },
    "2481": {
        "rookie_year": 2008,
        "location": "Illinois",
        "epa_rank": 45,
        "division": "Hopper"
    },
    "2486": {
        "rookie_year": 2008,
        "location": "Arizona",
        "epa_rank": 1096,
        "division": "Daly"
    },
    "2489": {
        "rookie_year": 2008,
        "location": "California",
        "epa_rank": 1158,
        "division": "Milstein"
    },
    "2491": {
        "rookie_year": 2008,
        "location": "Minnesota",
        "epa_rank": 703,
        "division": "Daly"
    },
    "2495": {
        "rookie_year": 2008,
        "location": "New Jersey",
        "epa_rank": 542,
        "division": "Hopper"
    },
    "2506": {
        "rookie_year": 2008,
        "location": "Wisconsin",
        "epa_rank": 966,
        "division": "Archimedes"
    },
    "2521": {
        "rookie_year": 2008,
        "location": "Oregon",
        "epa_rank": 78,
        "division": "Johnson"
    },
    "2539": {
        "rookie_year": 2008,
        "location": "Pennsylvania",
        "epa_rank": 31,
        "division": "Milstein"
    },
    "2551": {
        "rookie_year": 2008,
        "location": "California",
        "epa_rank": 419,
        "division": "Galileo"
    },
    "2582": {
        "rookie_year": 2008,
        "location": "Texas",
        "epa_rank": 583,
        "division": "Archimedes"
    },
    "2590": {
        "rookie_year": 2008,
        "location": "New Jersey",
        "epa_rank": 364,
        "division": "Daly"
    },
    "2607": {
        "rookie_year": 2008,
        "location": "Pennsylvania",
        "epa_rank": 197,
        "division": "Archimedes"
    },
    "2609": {
        "rookie_year": 2008,
        "location": "Canada",
        "epa_rank": 469,
        "division": "Hopper"
    },
    "2614": {
        "rookie_year": 2008,
        "location": "West Virginia",
        "epa_rank": 395,
        "division": "Galileo"
    },
    "2630": {
        "rookie_year": 2008,
        "location": "Israel",
        "epa_rank": 156,
        "division": "Galileo"
    },
    "2635": {
        "rookie_year": 2008,
        "location": "Oregon",
        "epa_rank": 1449,
        "division": "Milstein"
    },
    "2637": {
        "rookie_year": 2008,
        "location": "California",
        "epa_rank": 943,
        "division": "Curie"
    },
    "2638": {
        "rookie_year": 2008,
        "location": "New York",
        "epa_rank": 746,
        "division": "Milstein"
    },
    "2642": {
        "rookie_year": 2008,
        "location": "North Carolina",
        "epa_rank": 234,
        "division": "Newton"
    },
    "2656": {
        "rookie_year": 2008,
        "location": "Pennsylvania",
        "epa_rank": 959,
        "division": "Milstein"
    },
    "2659": {
        "rookie_year": 2008,
        "location": "California",
        "epa_rank": 60,
        "division": "Daly"
    },
    "2667": {
        "rookie_year": 2008,
        "location": "Minnesota",
        "epa_rank": 1113,
        "division": "Daly"
    },
    "2687": {
        "rookie_year": 2008,
        "location": "Texas",
        "epa_rank": 55,
        "division": "Archimedes"
    },
    "2713": {
        "rookie_year": 2009,
        "location": "Massachusetts",
        "epa_rank": 463,
        "division": "Daly"
    },
    "2714": {
        "rookie_year": 2009,
        "location": "Texas",
        "epa_rank": 346,
        "division": "Galileo"
    },
    "2718": {
        "rookie_year": 2009,
        "location": "Oklahoma",
        "epa_rank": 2126,
        "division": "Hopper"
    },
    "2767": {
        "rookie_year": 2009,
        "location": "Michigan",
        "epa_rank": 18,
        "division": "Daly"
    },
    "2771": {
        "rookie_year": 2009,
        "location": "Michigan",
        "epa_rank": 1098,
        "division": "Galileo"
    },
    "2791": {
        "rookie_year": 2009,
        "location": "New York",
        "epa_rank": 359,
        "division": "Curie"
    },
    "2830": {
        "rookie_year": 2009,
        "location": "Wisconsin",
        "epa_rank": 629,
        "division": "Galileo"
    },
    "2832": {
        "rookie_year": 2009,
        "location": "Michigan",
        "epa_rank": 496,
        "division": "Johnson"
    },
    "2834": {
        "rookie_year": 2009,
        "location": "Michigan",
        "epa_rank": 397,
        "division": "Galileo"
    },
    "2847": {
        "rookie_year": 2009,
        "location": "Minnesota",
        "epa_rank": 344,
        "division": "Archimedes"
    },
    "2869": {
        "rookie_year": 2009,
        "location": "New York",
        "epa_rank": 1257,
        "division": "Daly"
    },
    "2875": {
        "rookie_year": 2009,
        "location": "New York",
        "epa_rank": 721,
        "division": "Archimedes"
    },
    "2881": {
        "rookie_year": 2009,
        "location": "Texas",
        "epa_rank": 315,
        "division": "Galileo"
    },
    "2883": {
        "rookie_year": 2009,
        "location": "Minnesota",
        "epa_rank": 318,
        "division": "Daly"
    },
    "2905": {
        "rookie_year": 2009,
        "location": "Turkiye",
        "epa_rank": 2199,
        "division": "Archimedes"
    },
    "2910": {
        "rookie_year": 2009,
        "location": "Washington",
        "epa_rank": 24,
        "division": "Galileo"
    },
    "2930": {
        "rookie_year": 2009,
        "location": "Washington",
        "epa_rank": 139,
        "division": "Hopper"
    },
    "2945": {
        "rookie_year": 2009,
        "location": "Colorado",
        "epa_rank": 1091,
        "division": "Curie"
    },
    "2960": {
        "rookie_year": 2009,
        "location": "Michigan",
        "epa_rank": 682,
        "division": "Newton"
    },
    "2974": {
        "rookie_year": 2009,
        "location": "Georgia",
        "epa_rank": 91,
        "division": "Milstein"
    },
    "2992": {
        "rookie_year": 2009,
        "location": "Louisiana",
        "epa_rank": 126,
        "division": "Newton"
    },
    "3003": {
        "rookie_year": 2009,
        "location": "New York",
        "epa_rank": 619,
        "division": "Newton"
    },
    "3005": {
        "rookie_year": 2009,
        "location": "Texas",
        "epa_rank": 5,
        "division": "Galileo"
    },
    "3008": {
        "rookie_year": 2009,
        "location": "Hawaii",
        "epa_rank": 810,
        "division": "Curie"
    },
    "3015": {
        "rookie_year": 2009,
        "location": "New York",
        "epa_rank": 136,
        "division": "Archimedes"
    },
    "3019": {
        "rookie_year": 2009,
        "location": "Arizona",
        "epa_rank": 2864,
        "division": "Hopper"
    },
    "3035": {
        "rookie_year": 2009,
        "location": "Texas",
        "epa_rank": 341,
        "division": "Daly"
    },
    "3039": {
        "rookie_year": 2009,
        "location": "Louisiana",
        "epa_rank": 113,
        "division": "Newton"
    },
    "3042": {
        "rookie_year": 2009,
        "location": "Minnesota",
        "epa_rank": 1460,
        "division": "Galileo"
    },
    "3061": {
        "rookie_year": 2009,
        "location": "Illinois",
        "epa_rank": 271,
        "division": "Archimedes"
    },
    "3082": {
        "rookie_year": 2009,
        "location": "Minnesota",
        "epa_rank": 884,
        "division": "Daly"
    },
    "3130": {
        "rookie_year": 2010,
        "location": "Minnesota",
        "epa_rank": 644,
        "division": "Archimedes"
    },
    "3132": {
        "rookie_year": 2010,
        "location": "Australia",
        "epa_rank": 568,
        "division": "Curie"
    },
    "3142": {
        "rookie_year": 2010,
        "location": "New Jersey",
        "epa_rank": 613,
        "division": "Archimedes"
    },
    "3161": {
        "rookie_year": 2010,
        "location": "Canada",
        "epa_rank": 514,
        "division": "Newton"
    },
    "3175": {
        "rookie_year": 2010,
        "location": "Michigan",
        "epa_rank": 93,
        "division": "Johnson"
    },
    "3184": {
        "rookie_year": 2010,
        "location": "Minnesota",
        "epa_rank": 43,
        "division": "Newton"
    },
    "3189": {
        "rookie_year": 2010,
        "location": "California",
        "epa_rank": 1544,
        "division": "Hopper"
    },
    "3218": {
        "rookie_year": 2010,
        "location": "Washington",
        "epa_rank": 307,
        "division": "Newton"
    },
    "3229": {
        "rookie_year": 2010,
        "location": "North Carolina",
        "epa_rank": 1425,
        "division": "Galileo"
    },
    "3256": {
        "rookie_year": 2010,
        "location": "California",
        "epa_rank": 2085,
        "division": "Archimedes"
    },
    "3284": {
        "rookie_year": 2010,
        "location": "Missouri",
        "epa_rank": 954,
        "division": "Curie"
    },
    "3310": {
        "rookie_year": 2010,
        "location": "Texas",
        "epa_rank": 160,
        "division": "Curie"
    },
    "3314": {
        "rookie_year": 2010,
        "location": "New Jersey",
        "epa_rank": 46,
        "division": "Daly"
    },
    "3322": {
        "rookie_year": 2010,
        "location": "Michigan",
        "epa_rank": 768,
        "division": "Archimedes"
    },
    "3339": {
        "rookie_year": 2010,
        "location": "Israel",
        "epa_rank": 186,
        "division": "Johnson"
    },
    "3341": {
        "rookie_year": 2010,
        "location": "California",
        "epa_rank": 2168,
        "division": "Curie"
    },
    "3357": {
        "rookie_year": 2010,
        "location": "Michigan",
        "epa_rank": 88,
        "division": "Hopper"
    },
    "3374": {
        "rookie_year": 2010,
        "location": "Wyoming",
        "epa_rank": 670,
        "division": "Milstein"
    },
    "3390": {
        "rookie_year": 2010,
        "location": "Turkiye",
        "epa_rank": 1173,
        "division": "Daly"
    },
    "3414": {
        "rookie_year": 2010,
        "location": "Michigan",
        "epa_rank": 391,
        "division": "Daly"
    },
    "3459": {
        "rookie_year": 2011,
        "location": "North Carolina",
        "epa_rank": 2064,
        "division": "Archimedes"
    },
    "3467": {
        "rookie_year": 2011,
        "location": "New Hampshire",
        "epa_rank": 42,
        "division": "Galileo"
    },
    "3476": {
        "rookie_year": 2011,
        "location": "California",
        "epa_rank": 14,
        "division": "Galileo"
    },
    "3478": {
        "rookie_year": 2011,
        "location": "Mexico",
        "epa_rank": 295,
        "division": "Newton"
    },
    "3489": {
        "rookie_year": 2011,
        "location": "South Carolina",
        "epa_rank": 1447,
        "division": "Curie"
    },
    "3506": {
        "rookie_year": 2011,
        "location": "North Carolina",
        "epa_rank": 253,
        "division": "Johnson"
    },
    "3526": {
        "rookie_year": 2011,
        "location": "Mexico",
        "epa_rank": 1765,
        "division": "Curie"
    },
    "3527": {
        "rookie_year": 2011,
        "location": "Mexico",
        "epa_rank": 2380,
        "division": "Daly"
    },
    "3534": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 285,
        "division": "Galileo"
    },
    "3536": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 429,
        "division": "Galileo"
    },
    "3538": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 7,
        "division": "Newton"
    },
    "3539": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 369,
        "division": "Galileo"
    },
    "3572": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 289,
        "division": "Newton"
    },
    "3603": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 695,
        "division": "Galileo"
    },
    "3604": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 312,
        "division": "Milstein"
    },
    "3616": {
        "rookie_year": 2011,
        "location": "Louisiana",
        "epa_rank": 864,
        "division": "Curie"
    },
    "3620": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 237,
        "division": "Milstein"
    },
    "3636": {
        "rookie_year": 2011,
        "location": "Oregon",
        "epa_rank": 265,
        "division": "Hopper"
    },
    "3641": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 123,
        "division": "Daly"
    },
    "3647": {
        "rookie_year": 2011,
        "location": "California",
        "epa_rank": 172,
        "division": "Daly"
    },
    "3653": {
        "rookie_year": 2011,
        "location": "Florida",
        "epa_rank": 2013,
        "division": "Johnson"
    },
    "3654": {
        "rookie_year": 2011,
        "location": "Connecticut",
        "epa_rank": 1748,
        "division": "Hopper"
    },
    "3655": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 90,
        "division": "Milstein"
    },
    "3663": {
        "rookie_year": 2011,
        "location": "Washington",
        "epa_rank": 56,
        "division": "Archimedes"
    },
    "3668": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 188,
        "division": "Milstein"
    },
    "3679": {
        "rookie_year": 2011,
        "location": "Texas",
        "epa_rank": 821,
        "division": "Milstein"
    },
    "3683": {
        "rookie_year": 2011,
        "location": "Canada",
        "epa_rank": 49,
        "division": "Milstein"
    },
    "3749": {
        "rookie_year": 2011,
        "location": "California",
        "epa_rank": 654,
        "division": "Curie"
    },
    "3753": {
        "rookie_year": 2011,
        "location": "Louisiana",
        "epa_rank": 2482,
        "division": "Johnson"
    },
    "3767": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 475,
        "division": "Newton"
    },
    "3794": {
        "rookie_year": 2011,
        "location": "Mexico",
        "epa_rank": 1996,
        "division": "Johnson"
    },
    "3847": {
        "rookie_year": 2011,
        "location": "Texas",
        "epa_rank": 250,
        "division": "Curie"
    },
    "3875": {
        "rookie_year": 2011,
        "location": "Michigan",
        "epa_rank": 1353,
        "division": "Daly"
    },
    "3928": {
        "rookie_year": 2012,
        "location": "Iowa",
        "epa_rank": 633,
        "division": "Daly"
    },
    "3932": {
        "rookie_year": 2012,
        "location": "Florida",
        "epa_rank": 391,
        "division": "Newton"
    },
    "3937": {
        "rookie_year": 2012,
        "location": "Arkansas",
        "epa_rank": 79,
        "division": "Milstein"
    },
    "3940": {
        "rookie_year": 2012,
        "location": "Indiana",
        "epa_rank": 239,
        "division": "Newton"
    },
    "3990": {
        "rookie_year": 2012,
        "location": "Canada",
        "epa_rank": 153,
        "division": "Curie"
    },
    "3996": {
        "rookie_year": 2012,
        "location": "Canada",
        "epa_rank": 306,
        "division": "Galileo"
    },
    "4003": {
        "rookie_year": 2012,
        "location": "Michigan",
        "epa_rank": 162,
        "division": "Curie"
    },
    "4009": {
        "rookie_year": 2012,
        "location": "Minnesota",
        "epa_rank": 585,
        "division": "Galileo"
    },
    "4010": {
        "rookie_year": 2012,
        "location": "Mexico",
        "epa_rank": 435,
        "division": "Curie"
    },
    "4011": {
        "rookie_year": 2012,
        "location": "Wisconsin",
        "epa_rank": 600,
        "division": "Johnson"
    },
    "4020": {
        "rookie_year": 2012,
        "location": "Tennessee",
        "epa_rank": 141,
        "division": "Milstein"
    },
    "4028": {
        "rookie_year": 2012,
        "location": "Ohio",
        "epa_rank": 26,
        "division": "Archimedes"
    },
    "4039": {
        "rookie_year": 2012,
        "location": "Canada",
        "epa_rank": 81,
        "division": "Milstein"
    },
    "4069": {
        "rookie_year": 2012,
        "location": "Canada",
        "epa_rank": 385,
        "division": "Newton"
    },
    "4091": {
        "rookie_year": 2012,
        "location": "Dominican Republic",
        "epa_rank": 1288,
        "division": "Daly"
    },
    "4096": {
        "rookie_year": 2012,
        "location": "Illinois",
        "epa_rank": 388,
        "division": "Hopper"
    },
    "4099": {
        "rookie_year": 2012,
        "location": "Maryland",
        "epa_rank": 394,
        "division": "Newton"
    },
    "4112": {
        "rookie_year": 2012,
        "location": "Georgia",
        "epa_rank": 1352,
        "division": "Newton"
    },
    "4122": {
        "rookie_year": 2012,
        "location": "New York",
        "epa_rank": 676,
        "division": "Milstein"
    },
    "4131": {
        "rookie_year": 2012,
        "location": "Washington",
        "epa_rank": 352,
        "division": "Curie"
    },
    "4135": {
        "rookie_year": 2012,
        "location": "California",
        "epa_rank": 1109,
        "division": "Hopper"
    },
    "4143": {
        "rookie_year": 2012,
        "location": "Illinois",
        "epa_rank": 30,
        "division": "Newton"
    },
    "4145": {
        "rookie_year": 2012,
        "location": "Ohio",
        "epa_rank": 456,
        "division": "Newton"
    },
    "4175": {
        "rookie_year": 2012,
        "location": "Idaho",
        "epa_rank": 1248,
        "division": "Milstein"
    },
    "4201": {
        "rookie_year": 2012,
        "location": "California",
        "epa_rank": 107,
        "division": "Daly"
    },
    "4206": {
        "rookie_year": 2012,
        "location": "Texas",
        "epa_rank": 181,
        "division": "Daly"
    },
    "4213": {
        "rookie_year": 2012,
        "location": "Illinois",
        "epa_rank": 626,
        "division": "Hopper"
    },
    "4226": {
        "rookie_year": 2012,
        "location": "Minnesota",
        "epa_rank": 1186,
        "division": "Daly"
    },
    "4253": {
        "rookie_year": 2012,
        "location": "Taiwan",
        "epa_rank": 269,
        "division": "Curie"
    },
    "4265": {
        "rookie_year": 2012,
        "location": "Tennessee",
        "epa_rank": 65,
        "division": "Hopper"
    },
    "4270": {
        "rookie_year": 2012,
        "location": "Hawaii",
        "epa_rank": 132,
        "division": "Curie"
    },
    "4272": {
        "rookie_year": 2012,
        "location": "Indiana",
        "epa_rank": 86,
        "division": "Daly"
    },
    "4276": {
        "rookie_year": 2012,
        "location": "California",
        "epa_rank": 920,
        "division": "Johnson"
    },
    "4293": {
        "rookie_year": 2012,
        "location": "Colorado",
        "epa_rank": 685,
        "division": "Johnson"
    },
    "4329": {
        "rookie_year": 2012,
        "location": "Missouri",
        "epa_rank": 258,
        "division": "Newton"
    },
    "4336": {
        "rookie_year": 2012,
        "location": "Louisiana",
        "epa_rank": 238,
        "division": "Hopper"
    },
    "4342": {
        "rookie_year": 2012,
        "location": "Pennsylvania",
        "epa_rank": 212,
        "division": "Johnson"
    },
    "4362": {
        "rookie_year": 2012,
        "location": "Michigan",
        "epa_rank": 57,
        "division": "Archimedes"
    },
    "4373": {
        "rookie_year": 2012,
        "location": "Pennsylvania",
        "epa_rank": 421,
        "division": "Archimedes"
    },
    "4381": {
        "rookie_year": 2012,
        "location": "Michigan",
        "epa_rank": 76,
        "division": "Galileo"
    },
    "4391": {
        "rookie_year": 2012,
        "location": "Michigan",
        "epa_rank": 51,
        "division": "Hopper"
    },
    "4392": {
        "rookie_year": 2012,
        "location": "Michigan",
        "epa_rank": 556,
        "division": "Curie"
    },
    "4400": {
        "rookie_year": 2012,
        "location": "Mexico",
        "epa_rank": 410,
        "division": "Hopper"
    },
    "4414": {
        "rookie_year": 2012,
        "location": "California",
        "epa_rank": 11,
        "division": "Hopper"
    },
    "4415": {
        "rookie_year": 2012,
        "location": "California",
        "epa_rank": 166,
        "division": "Galileo"
    },
    "4419": {
        "rookie_year": 2012,
        "location": "California",
        "epa_rank": 565,
        "division": "Newton"
    },
    "4422": {
        "rookie_year": 2012,
        "location": "Michigan",
        "epa_rank": 276,
        "division": "Galileo"
    },
    "4450": {
        "rookie_year": 2013,
        "location": "Washington",
        "epa_rank": 2222,
        "division": "Hopper"
    },
    "4451": {
        "rookie_year": 2013,
        "location": "South Carolina",
        "epa_rank": 208,
        "division": "Galileo"
    },
    "4476": {
        "rookie_year": 2013,
        "location": "Canada",
        "epa_rank": 133,
        "division": "Archimedes"
    },
    "4481": {
        "rookie_year": 2013,
        "location": "Netherlands",
        "epa_rank": 98,
        "division": "Galileo"
    },
    "4499": {
        "rookie_year": 2013,
        "location": "Colorado",
        "epa_rank": 70,
        "division": "Johnson"
    },
    "4522": {
        "rookie_year": 2013,
        "location": "Missouri",
        "epa_rank": 39,
        "division": "Newton"
    },
    "4561": {
        "rookie_year": 2013,
        "location": "North Carolina",
        "epa_rank": 445,
        "division": "Milstein"
    },
    "4571": {
        "rookie_year": 2013,
        "location": "New York",
        "epa_rank": 1388,
        "division": "Galileo"
    },
    "4598": {
        "rookie_year": 2013,
        "location": "Utah",
        "epa_rank": 1743,
        "division": "Archimedes"
    },
    "4607": {
        "rookie_year": 2013,
        "location": "Minnesota",
        "epa_rank": 75,
        "division": "Johnson"
    },
    "4613": {
        "rookie_year": 2013,
        "location": "Australia",
        "epa_rank": 291,
        "division": "Daly"
    },
    "4624": {
        "rookie_year": 2013,
        "location": "Minnesota",
        "epa_rank": 584,
        "division": "Johnson"
    },
    "4635": {
        "rookie_year": 2013,
        "location": "Mexico",
        "epa_rank": 213,
        "division": "Johnson"
    },
    "4663": {
        "rookie_year": 2013,
        "location": "Minnesota",
        "epa_rank": 601,
        "division": "Newton"
    },
    "4682": {
        "rookie_year": 2013,
        "location": "Washington",
        "epa_rank": 506,
        "division": "Hopper"
    },
    "4693": {
        "rookie_year": 2013,
        "location": "Minnesota",
        "epa_rank": 1951,
        "division": "Milstein"
    },
    "4738": {
        "rookie_year": 2013,
        "location": "California",
        "epa_rank": 59,
        "division": "Curie"
    },
    "4766": {
        "rookie_year": 2013,
        "location": "Missouri",
        "epa_rank": 245,
        "division": "Curie"
    },
    "4776": {
        "rookie_year": 2013,
        "location": "Michigan",
        "epa_rank": 442,
        "division": "Johnson"
    },
    "4779": {
        "rookie_year": 2013,
        "location": "Michigan",
        "epa_rank": 516,
        "division": "Milstein"
    },
    "4786": {
        "rookie_year": 2013,
        "location": "Wisconsin",
        "epa_rank": 193,
        "division": "Galileo"
    },
    "4788": {
        "rookie_year": 2013,
        "location": "Australia",
        "epa_rank": 951,
        "division": "Curie"
    },
    "4795": {
        "rookie_year": 2013,
        "location": "North Carolina",
        "epa_rank": 179,
        "division": "Daly"
    },
    "4903": {
        "rookie_year": 2014,
        "location": "Canada",
        "epa_rank": 233,
        "division": "Archimedes"
    },
    "4905": {
        "rookie_year": 2014,
        "location": "Massachusetts",
        "epa_rank": 871,
        "division": "Newton"
    },
    "4907": {
        "rookie_year": 2014,
        "location": "Canada",
        "epa_rank": 147,
        "division": "Daly"
    },
    "4909": {
        "rookie_year": 2014,
        "location": "Massachusetts",
        "epa_rank": 227,
        "division": "Newton"
    },
    "4920": {
        "rookie_year": 2014,
        "location": "Canada",
        "epa_rank": 167,
        "division": "Johnson"
    },
    "4930": {
        "rookie_year": 2014,
        "location": "New York",
        "epa_rank": 1144,
        "division": "Hopper"
    },
    "4944": {
        "rookie_year": 2014,
        "location": "Colorado",
        "epa_rank": 404,
        "division": "Newton"
    },
    "4946": {
        "rookie_year": 2014,
        "location": "Canada",
        "epa_rank": 142,
        "division": "Daly"
    },
    "4976": {
        "rookie_year": 2014,
        "location": "Canada",
        "epa_rank": 248,
        "division": "Johnson"
    },
    "5006": {
        "rookie_year": 2014,
        "location": "Arkansas",
        "epa_rank": 371,
        "division": "Newton"
    },
    "5010": {
        "rookie_year": 2014,
        "location": "Indiana",
        "epa_rank": 203,
        "division": "Galileo"
    },
    "5024": {
        "rookie_year": 2014,
        "location": "Canada",
        "epa_rank": 502,
        "division": "Milstein"
    },
    "5045": {
        "rookie_year": 2014,
        "location": "Tennessee",
        "epa_rank": 3191,
        "division": "Archimedes"
    },
    "5050": {
        "rookie_year": 2014,
        "location": "Michigan",
        "epa_rank": 590,
        "division": "Johnson"
    },
    "5089": {
        "rookie_year": 2014,
        "location": "California",
        "epa_rank": 1252,
        "division": "Milstein"
    },
    "5123": {
        "rookie_year": 2014,
        "location": "New York",
        "epa_rank": 2242,
        "division": "Hopper"
    },
    "5135": {
        "rookie_year": 2014,
        "location": "Israel",
        "epa_rank": 229,
        "division": "Newton"
    },
    "5152": {
        "rookie_year": 2014,
        "location": "Michigan",
        "epa_rank": 246,
        "division": "Milstein"
    },
    "5166": {
        "rookie_year": 2014,
        "location": "Michigan",
        "epa_rank": 73,
        "division": "Galileo"
    },
    "5172": {
        "rookie_year": 2014,
        "location": "Minnesota",
        "epa_rank": 1084,
        "division": "Newton"
    },
    "5199": {
        "rookie_year": 2014,
        "location": "California",
        "epa_rank": 148,
        "division": "Galileo"
    },
    "5216": {
        "rookie_year": 2014,
        "location": "Michigan",
        "epa_rank": 210,
        "division": "Daly"
    },
    "5232": {
        "rookie_year": 2014,
        "location": "Minnesota",
        "epa_rank": 254,
        "division": "Archimedes"
    },
    "5274": {
        "rookie_year": 2014,
        "location": "California",
        "epa_rank": 769,
        "division": "Newton"
    },
    "5338": {
        "rookie_year": 2014,
        "location": "Virginia",
        "epa_rank": 812,
        "division": "Newton"
    },
    "5406": {
        "rookie_year": 2015,
        "location": "Canada",
        "epa_rank": 267,
        "division": "Daly"
    },
    "5409": {
        "rookie_year": 2015,
        "location": "Canada",
        "epa_rank": 376,
        "division": "Milstein"
    },
    "5411": {
        "rookie_year": 2015,
        "location": "Texas",
        "epa_rank": 288,
        "division": "Milstein"
    },
    "5414": {
        "rookie_year": 2015,
        "location": "Texas",
        "epa_rank": 157,
        "division": "Curie"
    },
    "5419": {
        "rookie_year": 2015,
        "location": "California",
        "epa_rank": 302,
        "division": "Milstein"
    },
    "5422": {
        "rookie_year": 2015,
        "location": "Massachusetts",
        "epa_rank": 2002,
        "division": "Daly"
    },
    "5427": {
        "rookie_year": 2015,
        "location": "Texas",
        "epa_rank": 672,
        "division": "Galileo"
    },
    "5431": {
        "rookie_year": 2015,
        "location": "Texas",
        "epa_rank": 878,
        "division": "Johnson"
    },
    "5434": {
        "rookie_year": 2015,
        "location": "Minnesota",
        "epa_rank": 499,
        "division": "Johnson"
    },
    "5460": {
        "rookie_year": 2015,
        "location": "Michigan",
        "epa_rank": 64,
        "division": "Daly"
    },
    "5472": {
        "rookie_year": 2015,
        "location": "Florida",
        "epa_rank": 137,
        "division": "Daly"
    },
    "5505": {
        "rookie_year": 2015,
        "location": "Michigan",
        "epa_rank": 452,
        "division": "Milstein"
    },
    "5526": {
        "rookie_year": 2015,
        "location": "Mexico",
        "epa_rank": 688,
        "division": "Hopper"
    },
    "5553": {
        "rookie_year": 2015,
        "location": "France",
        "epa_rank": 1136,
        "division": "Newton"
    },
    "5557": {
        "rookie_year": 2015,
        "location": "Florida",
        "epa_rank": 846,
        "division": "Galileo"
    },
    "5586": {
        "rookie_year": 2015,
        "location": "Wisconsin",
        "epa_rank": 348,
        "division": "Galileo"
    },
    "5587": {
        "rookie_year": 2015,
        "location": "Virginia",
        "epa_rank": 2763,
        "division": "Hopper"
    },
    "5618": {
        "rookie_year": 2015,
        "location": "Canada",
        "epa_rank": 1023,
        "division": "Milstein"
    },
    "5635": {
        "rookie_year": 2015,
        "location": "Israel",
        "epa_rank": 973,
        "division": "Archimedes"
    },
    "5648": {
        "rookie_year": 2015,
        "location": "Australia",
        "epa_rank": 2812,
        "division": "Hopper"
    },
    "5653": {
        "rookie_year": 2015,
        "location": "Minnesota",
        "epa_rank": 870,
        "division": "Milstein"
    },
    "5654": {
        "rookie_year": 2015,
        "location": "Israel",
        "epa_rank": 439,
        "division": "Johnson"
    },
    "5665": {
        "rookie_year": 2015,
        "location": "Turkiye",
        "epa_rank": 629,
        "division": "Newton"
    },
    "5667": {
        "rookie_year": 2015,
        "location": "Ohio",
        "epa_rank": 745,
        "division": "Hopper"
    },
    "5675": {
        "rookie_year": 2015,
        "location": "Michigan",
        "epa_rank": 603,
        "division": "Hopper"
    },
    "5687": {
        "rookie_year": 2015,
        "location": "Maine",
        "epa_rank": 209,
        "division": "Curie"
    },
    "5712": {
        "rookie_year": 2015,
        "location": "Michigan",
        "epa_rank": 224,
        "division": "Daly"
    },
    "5724": {
        "rookie_year": 2015,
        "location": "Virginia",
        "epa_rank": 536,
        "division": "Daly"
    },
    "5727": {
        "rookie_year": 2015,
        "location": "North Carolina",
        "epa_rank": 74,
        "division": "Curie"
    },
    "5801": {
        "rookie_year": 2016,
        "location": "Missouri",
        "epa_rank": 397,
        "division": "Galileo"
    },
    "5804": {
        "rookie_year": 2016,
        "location": "Virginia",
        "epa_rank": 176,
        "division": "Newton"
    },
    "5809": {
        "rookie_year": 2016,
        "location": "Missouri",
        "epa_rank": 1679,
        "division": "Archimedes"
    },
    "5816": {
        "rookie_year": 2016,
        "location": "Florida",
        "epa_rank": 3099,
        "division": "Hopper"
    },
    "5883": {
        "rookie_year": 2016,
        "location": "Poland",
        "epa_rank": 886,
        "division": "Johnson"
    },
    "5895": {
        "rookie_year": 2016,
        "location": "New Jersey",
        "epa_rank": 94,
        "division": "Milstein"
    },
    "5901": {
        "rookie_year": 2016,
        "location": "Michigan",
        "epa_rank": 406,
        "division": "Curie"
    },
    "5907": {
        "rookie_year": 2016,
        "location": "Michigan",
        "epa_rank": 302,
        "division": "Curie"
    },
    "5913": {
        "rookie_year": 2016,
        "location": "Minnesota",
        "epa_rank": 155,
        "division": "Milstein"
    },
    "5940": {
        "rookie_year": 2016,
        "location": "California",
        "epa_rank": 4,
        "division": "Curie"
    },
    "5985": {
        "rookie_year": 2016,
        "location": "Australia",
        "epa_rank": 821,
        "division": "Galileo"
    },
    "5987": {
        "rookie_year": 2016,
        "location": "Israel",
        "epa_rank": 150,
        "division": "Galileo"
    },
    "5988": {
        "rookie_year": 2016,
        "location": "Australia",
        "epa_rank": 1965,
        "division": "Milstein"
    },
    "5990": {
        "rookie_year": 2016,
        "location": "Israel",
        "epa_rank": 222,
        "division": "Newton"
    },
    "5993": {
        "rookie_year": 2016,
        "location": "Turkiye",
        "epa_rank": 1748,
        "division": "Archimedes"
    },
    "6002": {
        "rookie_year": 2016,
        "location": "Michigan",
        "epa_rank": 207,
        "division": "Archimedes"
    },
    "6024": {
        "rookie_year": 2016,
        "location": "India",
        "epa_rank": 1265,
        "division": "Hopper"
    },
    "6036": {
        "rookie_year": 2016,
        "location": "California",
        "epa_rank": 8,
        "division": "Archimedes"
    },
    "6045": {
        "rookie_year": 2016,
        "location": "Minnesota",
        "epa_rank": 119,
        "division": "Johnson"
    },
    "6078": {
        "rookie_year": 2016,
        "location": "Michigan",
        "epa_rank": 494,
        "division": "Archimedes"
    },
    "6081": {
        "rookie_year": 2016,
        "location": "Michigan",
        "epa_rank": 183,
        "division": "Daly"
    },
    "6090": {
        "rookie_year": 2016,
        "location": "Michigan",
        "epa_rank": 68,
        "division": "Milstein"
    },
    "6146": {
        "rookie_year": 2016,
        "location": "Minnesota",
        "epa_rank": 327,
        "division": "Johnson"
    },
    "6328": {
        "rookie_year": 2017,
        "location": "Massachusetts",
        "epa_rank": 109,
        "division": "Archimedes"
    },
    "6329": {
        "rookie_year": 2017,
        "location": "Maine",
        "epa_rank": 20,
        "division": "Curie"
    },
    "6348": {
        "rookie_year": 2017,
        "location": "Mexico",
        "epa_rank": 855,
        "division": "Curie"
    },
    "6352": {
        "rookie_year": 2017,
        "location": "Arizona",
        "epa_rank": 901,
        "division": "Milstein"
    },
    "6357": {
        "rookie_year": 2017,
        "location": "Texas",
        "epa_rank": 605,
        "division": "Hopper"
    },
    "6358": {
        "rookie_year": 2017,
        "location": "Idaho",
        "epa_rank": 1580,
        "division": "Curie"
    },
    "6377": {
        "rookie_year": 2017,
        "location": "Texas",
        "epa_rank": 165,
        "division": "Johnson"
    },
    "6413": {
        "rookie_year": 2017,
        "location": "Arizona",
        "epa_rank": 790,
        "division": "Curie"
    },
    "6420": {
        "rookie_year": 2017,
        "location": "Iowa",
        "epa_rank": 608,
        "division": "Milstein"
    },
    "6424": {
        "rookie_year": 2017,
        "location": "Missouri",
        "epa_rank": 108,
        "division": "Daly"
    },
    "6429": {
        "rookie_year": 2017,
        "location": "Turkiye",
        "epa_rank": 792,
        "division": "Hopper"
    },
    "6431": {
        "rookie_year": 2017,
        "location": "Turkiye",
        "epa_rank": 3270,
        "division": "Newton"
    },
    "6510": {
        "rookie_year": 2017,
        "location": "Australia",
        "epa_rank": 368,
        "division": "Daly"
    },
    "6517": {
        "rookie_year": 2017,
        "location": "Tennessee",
        "epa_rank": 1386,
        "division": "Hopper"
    },
    "6586": {
        "rookie_year": 2017,
        "location": "Arkansas",
        "epa_rank": 1767,
        "division": "Curie"
    },
    "6606": {
        "rookie_year": 2017,
        "location": "Mexico",
        "epa_rank": 1821,
        "division": "Newton"
    },
    "6647": {
        "rookie_year": 2017,
        "location": "Mexico",
        "epa_rank": 1281,
        "division": "Curie"
    },
    "6652": {
        "rookie_year": 2017,
        "location": "Mexico",
        "epa_rank": 2011,
        "division": "Curie"
    },
    "6657": {
        "rookie_year": 2017,
        "location": "California",
        "epa_rank": 1189,
        "division": "Newton"
    },
    "6672": {
        "rookie_year": 2017,
        "location": "Texas",
        "epa_rank": 44,
        "division": "Milstein"
    },
    "6702": {
        "rookie_year": 2017,
        "location": "Mexico",
        "epa_rank": 1255,
        "division": "Galileo"
    },
    "6721": {
        "rookie_year": 2017,
        "location": "Indiana",
        "epa_rank": 1176,
        "division": "Hopper"
    },
    "6722": {
        "rookie_year": 2017,
        "location": "Florida",
        "epa_rank": 106,
        "division": "Galileo"
    },
    "6800": {
        "rookie_year": 2018,
        "location": "Texas",
        "epa_rank": 127,
        "division": "Galileo"
    },
    "6806": {
        "rookie_year": 2018,
        "location": "New York",
        "epa_rank": 1421,
        "division": "Daly"
    },
    "6817": {
        "rookie_year": 2018,
        "location": "Missouri",
        "epa_rank": 1976,
        "division": "Newton"
    },
    "6823": {
        "rookie_year": 2018,
        "location": "Wisconsin",
        "epa_rank": 353,
        "division": "Milstein"
    },
    "6832": {
        "rookie_year": 2018,
        "location": "Mexico",
        "epa_rank": 1755,
        "division": "Hopper"
    },
    "6833": {
        "rookie_year": 2018,
        "location": "Arizona",
        "epa_rank": 1064,
        "division": "Johnson"
    },
    "6838": {
        "rookie_year": 2018,
        "location": "Turkiye",
        "epa_rank": 329,
        "division": "Daly"
    },
    "6865": {
        "rookie_year": 2018,
        "location": "Canada",
        "epa_rank": 1679,
        "division": "Milstein"
    },
    "6868": {
        "rookie_year": 2018,
        "location": "New York",
        "epa_rank": 1166,
        "division": "Curie"
    },
    "6909": {
        "rookie_year": 2018,
        "location": "Japan",
        "epa_rank": 2105,
        "division": "Newton"
    },
    "6919": {
        "rookie_year": 2018,
        "location": "Georgia",
        "epa_rank": 195,
        "division": "Johnson"
    },
    "6989": {
        "rookie_year": 2018,
        "location": "Turkiye",
        "epa_rank": 1515,
        "division": "Daly"
    },
    "6995": {
        "rookie_year": 2018,
        "location": "California",
        "epa_rank": 365,
        "division": "Curie"
    },
    "6998": {
        "rookie_year": 2018,
        "location": "Taiwan",
        "epa_rank": 1771,
        "division": "Curie"
    },
    "7018": {
        "rookie_year": 2018,
        "location": "Mexico",
        "epa_rank": 1357,
        "division": "Milstein"
    },
    "7021": {
        "rookie_year": 2018,
        "location": "Wisconsin",
        "epa_rank": 145,
        "division": "Curie"
    },
    "7042": {
        "rookie_year": 2018,
        "location": "California",
        "epa_rank": 1339,
        "division": "Curie"
    },
    "7072": {
        "rookie_year": 2018,
        "location": "Alabama",
        "epa_rank": 879,
        "division": "Newton"
    },
    "7153": {
        "rookie_year": 2018,
        "location": "Connecticut",
        "epa_rank": 510,
        "division": "Daly"
    },
    "7157": {
        "rookie_year": 2018,
        "location": "California",
        "epa_rank": 35,
        "division": "Johnson"
    },
    "7174": {
        "rookie_year": 2018,
        "location": "Michigan",
        "epa_rank": 378,
        "division": "Archimedes"
    },
    "7197": {
        "rookie_year": 2018,
        "location": "Michigan",
        "epa_rank": 473,
        "division": "Galileo"
    },
    "7211": {
        "rookie_year": 2018,
        "location": "Michigan",
        "epa_rank": 189,
        "division": "Archimedes"
    },
    "7220": {
        "rookie_year": 2018,
        "location": "Michigan",
        "epa_rank": 277,
        "division": "Curie"
    },
    "7258": {
        "rookie_year": 2018,
        "location": "Minnesota",
        "epa_rank": 273,
        "division": "Galileo"
    },
    "7285": {
        "rookie_year": 2018,
        "location": "Turkiye",
        "epa_rank": 101,
        "division": "Newton"
    },
    "7407": {
        "rookie_year": 2019,
        "location": "Connecticut",
        "epa_rank": 121,
        "division": "Hopper"
    },
    "7428": {
        "rookie_year": 2019,
        "location": "Alabama",
        "epa_rank": 1580,
        "division": "Newton"
    },
    "7444": {
        "rookie_year": 2019,
        "location": "Turkiye",
        "epa_rank": 522,
        "division": "Archimedes"
    },
    "7451": {
        "rookie_year": 2019,
        "location": "Georgia",
        "epa_rank": 1109,
        "division": "Galileo"
    },
    "7457": {
        "rookie_year": 2019,
        "location": "Indiana",
        "epa_rank": 111,
        "division": "Hopper"
    },
    "7460": {
        "rookie_year": 2019,
        "location": "Ohio",
        "epa_rank": 461,
        "division": "Johnson"
    },
    "7461": {
        "rookie_year": 2019,
        "location": "Washington",
        "epa_rank": 440,
        "division": "Galileo"
    },
    "7534": {
        "rookie_year": 2019,
        "location": "Texas",
        "epa_rank": 2150,
        "division": "Curie"
    },
    "7558": {
        "rookie_year": 2019,
        "location": "Canada",
        "epa_rank": 223,
        "division": "Johnson"
    },
    "7563": {
        "rookie_year": 2019,
        "location": "Brazil",
        "epa_rank": 574,
        "division": "Archimedes"
    },
    "7565": {
        "rookie_year": 2019,
        "location": "Brazil",
        "epa_rank": 468,
        "division": "Curie"
    },
    "7617": {
        "rookie_year": 2019,
        "location": "Indiana",
        "epa_rank": 286,
        "division": "Newton"
    },
    "7657": {
        "rookie_year": 2019,
        "location": "Indiana",
        "epa_rank": 641,
        "division": "Galileo"
    },
    "7748": {
        "rookie_year": 2019,
        "location": "Turkiye",
        "epa_rank": 1421,
        "division": "Johnson"
    },
    "7769": {
        "rookie_year": 2019,
        "location": "Michigan",
        "epa_rank": 174,
        "division": "Daly"
    },
    "7890": {
        "rookie_year": 2019,
        "location": "North Carolina",
        "epa_rank": 146,
        "division": "Curie"
    },
    "8013": {
        "rookie_year": 2020,
        "location": "Massachusetts",
        "epa_rank": 216,
        "division": "Galileo"
    },
    "8015": {
        "rookie_year": 2020,
        "location": "China",
        "epa_rank": 2359,
        "division": "Newton"
    },
    "8016": {
        "rookie_year": 2020,
        "location": "California",
        "epa_rank": 1002,
        "division": "Newton"
    },
    "8019": {
        "rookie_year": 2020,
        "location": "Texas",
        "epa_rank": 714,
        "division": "Hopper"
    },
    "8033": {
        "rookie_year": 2020,
        "location": "California",
        "epa_rank": 100,
        "division": "Galileo"
    },
    "8046": {
        "rookie_year": 2020,
        "location": "New Hampshire",
        "epa_rank": 225,
        "division": "Curie"
    },
    "8085": {
        "rookie_year": 2020,
        "location": "Connecticut",
        "epa_rank": 175,
        "division": "Archimedes"
    },
    "8159": {
        "rookie_year": 2020,
        "location": "Turkiye",
        "epa_rank": 1847,
        "division": "Archimedes"
    },
    "8177": {
        "rookie_year": 2020,
        "location": "Texas",
        "epa_rank": 262,
        "division": "Curie"
    },
    "8393": {
        "rookie_year": 2020,
        "location": "Pennsylvania",
        "epa_rank": 454,
        "division": "Curie"
    },
    "8426": {
        "rookie_year": 2020,
        "location": "Michigan",
        "epa_rank": 544,
        "division": "Curie"
    },
    "8575": {
        "rookie_year": 2021,
        "location": "South Carolina",
        "epa_rank": 1920,
        "division": "Daly"
    },
    "8576": {
        "rookie_year": 2021,
        "location": "Texas",
        "epa_rank": 934,
        "division": "Milstein"
    },
    "8592": {
        "rookie_year": 2021,
        "location": "Virginia",
        "epa_rank": 189,
        "division": "Newton"
    },
    "8711": {
        "rookie_year": 2022,
        "location": "Oklahoma",
        "epa_rank": 1952,
        "division": "Curie"
    },
    "8717": {
        "rookie_year": 2022,
        "location": "Nevada",
        "epa_rank": 1433,
        "division": "Newton"
    },
    "8724": {
        "rookie_year": 2022,
        "location": "New Hampshire",
        "epa_rank": 287,
        "division": "Archimedes"
    },
    "8729": {
        "rookie_year": 2022,
        "location": "Canada",
        "epa_rank": 1086,
        "division": "Archimedes"
    },
    "8736": {
        "rookie_year": 2022,
        "location": "Georgia",
        "epa_rank": 615,
        "division": "Hopper"
    },
    "8739": {
        "rookie_year": 2022,
        "location": "New York",
        "epa_rank": 1323,
        "division": "Curie"
    },
    "8808": {
        "rookie_year": 2022,
        "location": "Louisiana",
        "epa_rank": 859,
        "division": "Newton"
    },
    "8847": {
        "rookie_year": 2022,
        "location": "Wisconsin",
        "epa_rank": 534,
        "division": "Newton"
    },
    "8871": {
        "rookie_year": 2022,
        "location": "Hawaii",
        "epa_rank": 2306,
        "division": "Johnson"
    },
    "9000": {
        "rookie_year": 2023,
        "location": "North Carolina",
        "epa_rank": 1041,
        "division": "Daly"
    },
    "9007": {
        "rookie_year": 2023,
        "location": "Oklahoma",
        "epa_rank": 1399,
        "division": "Milstein"
    },
    "9008": {
        "rookie_year": 2023,
        "location": "North Carolina",
        "epa_rank": 2361,
        "division": "Milstein"
    },
    "9015": {
        "rookie_year": 2023,
        "location": "New Jersey",
        "epa_rank": 886,
        "division": "Curie"
    },
    "9016": {
        "rookie_year": 2023,
        "location": "New York",
        "epa_rank": 1270,
        "division": "Curie"
    },
    "9019": {
        "rookie_year": 2023,
        "location": "Hawaii",
        "epa_rank": 2840,
        "division": "Curie"
    },
    "9022": {
        "rookie_year": 2023,
        "location": "Pennsylvania",
        "epa_rank": 775,
        "division": "Milstein"
    },
    "9023": {
        "rookie_year": 2023,
        "location": "Washington",
        "epa_rank": 1792,
        "division": "Newton"
    },
    "9030": {
        "rookie_year": 2023,
        "location": "Azerbaijan",
        "epa_rank": 1726,
        "division": "Newton"
    },
    "9031": {
        "rookie_year": 2023,
        "location": "Missouri",
        "epa_rank": 2057,
        "division": "Galileo"
    },
    "9037": {
        "rookie_year": 2023,
        "location": "Utah",
        "epa_rank": 1659,
        "division": "Johnson"
    },
    "9038": {
        "rookie_year": 2023,
        "location": "California",
        "epa_rank": 608,
        "division": "Archimedes"
    },
    "9059": {
        "rookie_year": 2023,
        "location": "Arizona",
        "epa_rank": 2196,
        "division": "Archimedes"
    },
    "9062": {
        "rookie_year": 2023,
        "location": "Canada",
        "epa_rank": 2852,
        "division": "Newton"
    },
    "9067": {
        "rookie_year": 2023,
        "location": "Arkansas",
        "epa_rank": 912,
        "division": "Archimedes"
    },
    "9068": {
        "rookie_year": 2023,
        "location": "Colorado",
        "epa_rank": 1567,
        "division": "Johnson"
    },
    "9071": {
        "rookie_year": 2023,
        "location": "Indiana",
        "epa_rank": 1638,
        "division": "Hopper"
    },
    "9072": {
        "rookie_year": 2023,
        "location": "Maryland",
        "epa_rank": 2587,
        "division": "Galileo"
    },
    "9075": {
        "rookie_year": 2023,
        "location": "Ohio",
        "epa_rank": 1334,
        "division": "Milstein"
    },
    "9076": {
        "rookie_year": 2023,
        "location": "Canada",
        "epa_rank": 1789,
        "division": "Milstein"
    },
    "9079": {
        "rookie_year": 2023,
        "location": "Taiwan",
        "epa_rank": 1303,
        "division": "Daly"
    },
    "9082": {
        "rookie_year": 2023,
        "location": "Iowa",
        "epa_rank": 1238,
        "division": "Hopper"
    },
    "9084": {
        "rookie_year": 2023,
        "location": "California",
        "epa_rank": 334,
        "division": "Newton"
    },
    "9085": {
        "rookie_year": 2023,
        "location": "Brazil",
        "epa_rank": 2210,
        "division": "Curie"
    },
    "9092": {
        "rookie_year": 2023,
        "location": "Iowa",
        "epa_rank": 899,
        "division": "Johnson"
    },
    "9097": {
        "rookie_year": 2023,
        "location": "Ohio",
        "epa_rank": 2930,
        "division": "Galileo"
    },
    "9101": {
        "rookie_year": 2023,
        "location": "Vermont",
        "epa_rank": 957,
        "division": "Archimedes"
    },
    "9105": {
        "rookie_year": 2023,
        "location": "Texas",
        "epa_rank": 504,
        "division": "Galileo"
    },
    "9118": {
        "rookie_year": 2023,
        "location": "Mexico",
        "epa_rank": 1564,
        "division": "Hopper"
    },
    "9120": {
        "rookie_year": 2023,
        "location": "Mexico",
        "epa_rank": 2212,
        "division": "Galileo"
    },
    "9125": {
        "rookie_year": 2023,
        "location": "California",
        "epa_rank": 483,
        "division": "Milstein"
    },
    "9126": {
        "rookie_year": 2023,
        "location": "Taiwan",
        "epa_rank": 1630,
        "division": "Newton"
    },
    "9128": {
        "rookie_year": 2023,
        "location": "Texas",
        "epa_rank": 252,
        "division": "Galileo"
    },
    "9134": {
        "rookie_year": 2023,
        "location": "Mexico",
        "epa_rank": 1305,
        "division": "Archimedes"
    },
    "9135": {
        "rookie_year": 2023,
        "location": "Suriname",
        "epa_rank": 1525,
        "division": "Daly"
    },
    "9136": {
        "rookie_year": 2023,
        "location": "Texas",
        "epa_rank": 519,
        "division": "Daly"
    },
    "9138": {
        "rookie_year": 2023,
        "location": "Utah",
        "epa_rank": 1286,
        "division": "Galileo"
    },
    "9140": {
        "rookie_year": 2023,
        "location": "Texas",
        "epa_rank": 479,
        "division": "Newton"
    },
    "9148": {
        "rookie_year": 2023,
        "location": "Michigan",
        "epa_rank": 2897,
        "division": "Archimedes"
    },
    "9152": {
        "rookie_year": 2023,
        "location": "Kentucky",
        "epa_rank": 1711,
        "division": "Johnson"
    },
    "9153": {
        "rookie_year": 2023,
        "location": "Louisiana",
        "epa_rank": 2518,
        "division": "Curie"
    },
    "9168": {
        "rookie_year": 2023,
        "location": "Brazil",
        "epa_rank": 1216,
        "division": "Curie"
    },
    "9176": {
        "rookie_year": 2023,
        "location": "Michigan",
        "epa_rank": 791,
        "division": "Johnson"
    },
    "9180": {
        "rookie_year": 2023,
        "location": "Canada",
        "epa_rank": 1665,
        "division": "Archimedes"
    },
    "9182": {
        "rookie_year": 2023,
        "location": "Michigan",
        "epa_rank": 1194,
        "division": "Hopper"
    },
    "9202": {
        "rookie_year": 2023,
        "location": "California",
        "epa_rank": 1395,
        "division": "Hopper"
    },
    "9213": {
        "rookie_year": 2023,
        "location": "Mexico",
        "epa_rank": 1258,
        "division": "Johnson"
    },
    "9219": {
        "rookie_year": 2023,
        "location": "Brazil",
        "epa_rank": 1443,
        "division": "Curie"
    },
    "9244": {
        "rookie_year": 2023,
        "location": "Belize",
        "epa_rank": 2250,
        "division": "Milstein"
    },
    "9260": {
        "rookie_year": 2023,
        "location": "Georgia",
        "epa_rank": 787,
        "division": "Hopper"
    },
    "9280": {
        "rookie_year": 2023,
        "location": "Mexico",
        "epa_rank": 1391,
        "division": "Daly"
    },
    "9287": {
        "rookie_year": 2023,
        "location": "Nevada",
        "epa_rank": 799,
        "division": "Daly"
    },
    "9293": {
        "rookie_year": 2023,
        "location": "Georgia",
        "epa_rank": 1273,
        "division": "Daly"
    },
    "9312": {
        "rookie_year": 2023,
        "location": "Michigan",
        "epa_rank": 102,
        "division": "Johnson"
    }
}

const emojis = {
    "-1":" 🔻",
    "0":" ✅",
    "1":" 🔺"
}

const light_green = '#90EE90';
const light_red = '#FFCCCC';

var attempts = 0;
var guesses = [];

function shakeInput() {
    const input = document.getElementById('teamInput');
    input.classList.add('shake');
    setTimeout(function() {
      input.classList.remove('shake');
    }, 300);
}

function validGuess(guess) {
    return frc_teams.hasOwnProperty(guess) && !guesses.includes(guess);
}

function chooseTeam() {
    const keys = Object.keys(frc_teams);
    const randomIndex = Math.floor(Math.random() * keys.length);
    return keys[randomIndex];
}

function addTableRow(teamData, cellColors) {
    const table = document.getElementById('dataTable');
    const tbody = table.querySelector('tbody');
    const newRow = document.createElement('tr');
    teamData.forEach((value, index) => {
      const cell = document.createElement('td');
      cell.textContent = value;
      if (cellColors && cellColors[index]) {
        cell.style.backgroundColor = cellColors[index];
      }
      newRow.appendChild(cell);
    });
    tbody.appendChild(newRow);
}
  
function compareIntegers(correct_team, guessed_team) {
    return correct_team<guessed_team ? -1 : correct_team>guessed_team ? 1 : 0;
}

function showPopup() {
    const popupContainer = document.createElement('div');
    popupContainer.classList.add('popup-container');
    const popupContent = document.createElement('div');
    popupContent.classList.add('popup-content');
    const message = document.createElement('span');
    message.classList.add('popup-message');
    message.textContent = 'Congratulations! You guessed the team in ' + attempts + (attempts > 1 ? ' tries' : ' try');
    const closeButton = document.createElement('button');
    closeButton.textContent = 'Close';
    closeButton.addEventListener('click', closePopup);
    const playAgainButton = document.createElement('button');
    playAgainButton.textContent = 'Play Again';
    playAgainButton.addEventListener('click', reloadPage);
    popupContent.appendChild(message);
    popupContent.appendChild(closeButton);
    popupContent.appendChild(playAgainButton);
    popupContainer.appendChild(popupContent);
    document.body.appendChild(popupContainer);
}
  
function closePopup() {
    const popupContainer = document.querySelector('.popup-container');
    if (popupContainer) {
        popupContainer.remove();
    }
}

function reloadPage() {
    location.reload();
}
  
var correctTeam = chooseTeam()
var rookie_year_correct = frc_teams[correctTeam].rookie_year
var location_correct = frc_teams[correctTeam].location;
var epa_correct = frc_teams[correctTeam].epa_rank;
var division_correct = frc_teams[correctTeam].division;

function guessAttempted(){
    var guess = document.getElementById('teamInput').value;
    if(!validGuess(guess)){
        shakeInput();
        return;
    }
    attempts+=1;
    guesses.push(guess);
    var rookie_year_guessed = frc_teams[guess].rookie_year
    var location_guessed = frc_teams[guess].location;
    var epa_guessed = frc_teams[guess].epa_rank;
    var division_guessed = frc_teams[guess].division;
    var location_matches = location_guessed==location_correct;
    var division_matches = division_guessed==division_correct;
    var younger_team = compareIntegers(rookie_year_correct, rookie_year_guessed);
    var epa_rank_higher = - compareIntegers(epa_correct, epa_guessed);
    addTableRow(
        [
            guess,
            division_guessed,
            location_guessed,
            epa_guessed+emojis[epa_rank_higher],
            rookie_year_guessed+emojis[younger_team]
        ],
        [
            null,
            division_matches?light_green:light_red,
            location_matches?light_green:light_red,
            null,
            null
        ]
    )
    document.getElementById('teamInput').value = "";
    if(division_matches && location_matches && younger_team==0 && epa_rank_higher==0){
        showPopup();
    }
}
